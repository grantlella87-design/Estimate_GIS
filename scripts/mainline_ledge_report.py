...
def _clean_source_value(value: object) -> str:
    ...
    if "<a " in lower and "href=" in lower:
        import html
        href_index = lower.find("href=")
        remainder = cleaned[href_index + 5:].lstrip() if href_index >= 0 else cleaned
        if remainder:
            quote = remainder[0]
            if quote in ("'", '"'):
                end_index = remainder.find(quote, 1)
                cleaned = remainder[1:end_index] if end_index > 0 else remainder[1:]
            else:
                cleaned = remainder.split()[0]
        cleaned = html.unescape(cleaned)
    cleaned = cleaned.strip().strip('"').strip("'")
    return cleaned

def _source_requires_sign_in(value: object) -> bool:
    "Only internal/National Grid ArcGIS sources should receive the auth.py token."
...
def _bounds_for_public_arcgis_query(bounds, bounds_crs, ledge_source):
    "Return EPSG-coded bounds for public MassGIS/ArcGIS Server URL queries."
    if bounds is None or _source_requires_sign_in(ledge_source):
        return bounds, bounds_crs
    from pyproj import CRS, Transformer
    from shapely.geometry import box
    from shapely.ops import transform
    source_crs = CRS.from_user_input(bounds_crs)
    target_crs = CRS.from_epsg(26986)
    if source_crs.to_epsg() == 26986:
        return bounds, "EPSG:26986"
    transformer = Transformer.from_crs(source_crs, target_crs, always_xy=True)
    geom = transform(transformer.transform, box(*bounds))
    query_bounds = tuple(float(v) for v in geom.bounds)
    progress(f"Public ArcGIS bounds transformed to EPSG:26986: {query_bounds}")
    return query_bounds, "EPSG:26986"
...
def load_ledge(args, bounds, bounds_crs, sign_in: bool = True) -> tuple[gpd.GeoDataFrame, str, str]:
    ...
    ledge_key = str(ledge_source).lower()
    if ledge_key in ("massgis", "massgis:surfgeo", "default"):
        ...
        return (
            gdf,
            args.ledge_profile,
            "MassGIS Surficial Geology (1:24,000): " + massgis_ledge.describe_profile(args.ledge_profile),
        )
    if ledge_key in ("agol", "agol:250k", "agol250k"):
        return _load_agol_ledge(args, bounds, bounds_crs, cache_dir)
    source_sign_in = _source_requires_sign_in(ledge_source) if sign_in else False
    query_bounds, query_bounds_crs = _bounds_for_public_arcgis_query(bounds, bounds_crs, ledge_source)
    progress(f"Loading ledge source: {ledge_source}")
    progress(f"Ledge source requires sign-in: {source_sign_in}")
    gdf = vector_source.read_source(
        ledge_source,
        layer=args.ledge_layer,
        bounds=query_bounds,
        bounds_crs=query_bounds_crs,
        workers=args.workers,
        batch_size=args.batch_size,
        sign_in=source_sign_in,
    )
