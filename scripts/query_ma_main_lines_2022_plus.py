...
def parse_args(argv=None):
    ...
    parser.add_argument(
        "--layer-url", default=DEFAULT_LAYER_URL, help="ArcGIS REST layer URL."
    )
    parser.add_argument("--out-dir", default="outputs", help="Output folder.")
    parser.add_argument(
        "--basename",
        default="ma_main_lines_2022_plus",
    ...
    parser.add_argument("--no-geojson", action="store_true", help="Write only the CSV.")
    parser.add_argument(
        "--count-only",
        action="store_true",
    ...
    parser.add_argument(
        "--chunk-size", type=int, default=500, help="ObjectIDs per query. Default: 500."
    )
...
def choose_where(session, layer_url, install_field, creation_field, since_date):
    ...
    if accepted_but_empty:
        log(
            "No date syntax matched any rows; the layer may have nothing since "
            f"{since_date}."
        )
...
def resolve_out_fields(metadata, curated):
    ...
    lookup = {str(name).lower(): name for name in available}
    resolved = [
        lookup[name.lower()] for name in CURATED_FIELDS if name.lower() in lookup
    ]
...
def post_query_json(session, query_url, params):
    ...
    payload.setdefault("token", auth.get_arcgis_token(session))
    response = session.post(
        query_url,
        data=payload,
        timeout=getattr(config, "REQUEST_TIMEOUT_SECONDS", 120),
...
def fetch_features_by_where(
    ...
    """Fetch features directly from the same WHERE clause using server pagination."""
    query_url = f"{layer_url.rstrip('/')}/query"
    features = []
    offset = 0
    page = 1
    while True:
        data = post_query_json(
            session,
            query_url,
            {
                "where": where,
        ...
        log(f"Page {page:,}: {len(features):,}/{expected_count:,}")
...
def write_csv(features, path, date_fields):
    ...
    with open(path, "w", newline="", encoding="utf-8-sig") as handle:
        ...
        for feature in features:
            writer.writerow(
                normalize_attributes(feature.get("attributes"), date_fields)
            )
        ...
        written.append(
            {
                ...
                "properties": normalize_attributes(
                    feature.get("attributes"), date_fields
                ),
            }
        )
    with open(path, "w", encoding="utf-8") as handle:
        json.dump(
            {"type": "FeatureCollection", "features": written},
            handle,
            ensure_ascii=False,
        )
    if skipped:
...
def main(argv=None):
    ...
    where, count = choose_where(
        session, args.layer_url, install_field, creation_field, since
    )
    ...
    features = fetch_features_by_where(
        session=session,
        layer_url=args.layer_url,
        where=where,
        out_fields=out_fields,
        page_size=max(1, args.chunk_size),
        with_geometry=not args.no_geojson,
