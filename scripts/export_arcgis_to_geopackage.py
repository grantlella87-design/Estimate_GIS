...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Fast ArcGIS REST export to GeoPackage using GeoPandas."
    )
    parser.add_argument(
        "--layer-url",
        required=True,
        help="ArcGIS MapServer/FeatureServer layer URL without /query.",
    )
    parser.add_argument(
        "--where",
        default="1=1",
        help="ArcGIS WHERE clause. The same WHERE is used for ID selection and download.",
    )
    parser.add_argument(
        "--out-fields", default="*", help="Comma-separated field list or *."
    )
    parser.add_argument("--out-gpkg", required=True, help="Output GeoPackage path.")
    parser.add_argument(
        "--layer-name", default="arcgis_export", help="GeoPackage layer name."
    )
    parser.add_argument(
        "--token-env",
    ...
    parser.add_argument(
        "--batch-size", type=int, default=500, help="OBJECTID batch size."
    )
    ...
    print(
        f"Exported {len(gdf):,} features to {args.out_gpkg} layer {args.layer_name}",
        flush=True,
    )
